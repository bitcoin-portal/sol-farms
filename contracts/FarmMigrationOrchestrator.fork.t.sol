...
contract FarmMigrationOrchestratorForkTest is Test {
    ...
    function testDirectAddLiquidityProportional() public {
        // Test direct addLiquidityProportional without any swaps
        console2.log("Testing direct addLiquidityProportional...");
        ...
        // Give user1 some VERSE and tBTC tokens
        uint256 verseAmount = 1000 * 1e18; // 1000 VERSE
        uint256 tbtcAmount = 1 * 1e18;     // 1 tBTC
        ...
        verseToken.transfer(address(orchestratorV3), verseAmount);
        tbtcToken.transfer(address(orchestratorV3), tbtcAmount);

        console2.log("Tokens transferred to orchestrator");
        console2.log("VERSE balance:", verseToken.balanceOf(address(orchestratorV3)));
        console2.log("tBTC balance:", tbtcToken.balanceOf(address(orchestratorV3)));

        // Approve Permit2 for both tokens (this is what the orchestrator does internally)
        vm.stopPrank();
        vm.startPrank(address(orchestratorV3));

        verseToken.approve(0x000000000022D473030F116dDEE9F6B43aC78BA3, type(uint256).max); // PERMIT2
        tbtcToken.approve(0x000000000022D473030F116dDEE9F6B43aC78BA3, type(uint256).max); // PERMIT2
        ...
        address[] memory poolTokens = orchestratorV3.balancerPool().getTokens();
        ...
        if (poolTokens[0] == address(verseToken)) {
            maxAmountsIn[0] = verseAmount;  // VERSE first
            maxAmountsIn[1] = tbtcAmount;   // tBTC second
        } else {
            maxAmountsIn[0] = tbtcAmount;   // tBTC first
            maxAmountsIn[1] = verseAmount;  // VERSE second
        }

        console2.log("maxAmountsIn[0]:", maxAmountsIn[0]);
        console2.log("maxAmountsIn[1]:", maxAmountsIn[1]);

        // Query the expected LP tokens
        uint256 expectedLpTokens = orchestratorV3.balancerRouter().queryAddLiquidityProportional(
            address(orchestratorV3.balancerPool()),
            maxAmountsIn,
            address(orchestratorV3),
            ""
        );

        console2.log("Expected LP tokens from query:", expectedLpTokens);

        // Now try to add liquidity with the queried amount
        uint256[] memory amountsIn = orchestratorV3.balancerRouter().addLiquidityProportional(
            address(orchestratorV3.balancerPool()),
            maxAmountsIn,
            expectedLpTokens,
            false, // wethIsEth
        ...
        console2.log("Actual amountsIn[0]:", amountsIn[0]);
        console2.log("Actual amountsIn[1]:", amountsIn[1]);

        // Check LP tokens received
        uint256 lpTokensReceived = lpToken.balanceOf(address(orchestratorV3));
        console2.log("LP tokens received:", lpTokensReceived);

        vm.stopPrank();
    }

    function testAddLiquiditySingle() public {
        // Test the new addLiquiditySingle function
        console.log("Testing addLiquiditySingle function...");
        ...
        // Give user1 some VERSE tokens (using amounts from successful transaction)
        uint256 verseAmount = 105719376787168668206825; // VERSE amount from successful tx
        uint256 exactLpTokensOut = 1 * 1e18; // Want exactly 1 LP token
        ...
        // Call addLiquiditySingle
        uint256 actualVerseIn = orchestratorV3.addLiquiditySingle(exactLpTokensOut, verseAmount);
        ...
        console.log("  VERSE:", finalVerse);
        console.log("  LP tokens:", finalLp);
        console.log("Actual VERSE used:", actualVerseIn);

        // Key assertions
        assertGt(finalLp, 0, "User should have LP tokens!");
        assertGt(actualVerseIn, 0, "Should have used some VERSE!");
        assertLt(actualVerseIn, verseAmount, "Should not use more than max VERSE!");

        console.log("addLiquiditySingle test passed!");

        vm.stopPrank();
    }

    function testAddLiquidityExactOut() public {
        // Test the new addLiquidityExactOut function
        console.log("Testing addLiquidityExactOut function...");
        ...
        uint256 verseAmount = 1000 * 1e18; // 1000 VERSE
        uint256 tbtcAmount = 1 * 1e18;     // 1 tBTC
        uint256 exactLpTokensOut = 5 * 1e18; // Want exactly 5 LP tokens
        ...
        (uint256 tbtcBalance, uint256 verseBalance) = orchestratorV3.getPoolBalances();
        uint256 lpTotalSupply = orchestratorV3.balancerPool().totalSupply();
        console.log("Pool state:");
        console.log("  tBTC balance:", tbtcBalance);
        console.log("  VERSE balance:", verseBalance);
        console.log("  LP total supply:", lpTotalSupply);

        // Calculate amounts needed for 5 LP tokens
        (uint256 neededVerse, uint256 neededTbtc) = orchestratorV3.calculateAmountsForLpTokens(exactLpTokensOut);
        console.log("Calculated amounts for", exactLpTokensOut, "LP tokens:");
        console.log("  Needed VERSE:", neededVerse);
        console.log("  Needed tBTC:", neededTbtc);

        // Approve orchestrator to spend tokens
        verseToken.approve(address(orchestratorV3), verseAmount);
        tbtcToken.approve(address(orchestratorV3), tbtcAmount);

        // Call addLiquidityExactOut
        (uint256 actualVerseIn, uint256 actualTbtcIn) = orchestratorV3.addLiquidityExactOut(exactLpTokensOut, verseAmount, tbtcAmount);
        ...
        console.log("  LP tokens:", finalLp);
        console.log("Actual VERSE used:", actualVerseIn);
        console.log("Actual tBTC used:", actualTbtcIn);

        // Key assertions
        assertGt(finalLp, 0, "User should have LP tokens!");
        assertGt(actualVerseIn, 0, "Should have used some VERSE!");
        assertGt(actualTbtcIn, 0, "Should have used some tBTC!");

        console.log("addLiquidityExactOut test passed!");

        vm.stopPrank();
    }

    function testQueryAddLiquidityProportional() public {
        // Test the query function with the correct struct-based interface
        console.log("Testing queryAddLiquidityProportional with struct...");
        ...
        // Test amounts (80/20 ratio)
        uint256 verseAmount = 80 * 1e18; // 80 VERSE
        uint256 tbtcAmount = 20 * 1e18;  // 20 tBTC

        console.log("Testing with amounts:");
        console.log("  VERSE:", verseAmount);
        console.log("  tBTC:", tbtcAmount);

        // Get pool tokens to determine order
        address[] memory poolTokens = orchestratorV3.balancerPool().getTokens();
        console.log("Pool tokens[0]:", poolTokens[0]);
        console.log("Pool tokens[1]:", poolTokens[1]);
        ...
        console.log("maxAmountsIn[0]:", maxAmountsIn[0]);
        console.log("maxAmountsIn[1]:", maxAmountsIn[1]);

        // Create the request struct
        IBalancerV3Router.AddLiquidityProportionalRequest memory req = IBalancerV3Router.AddLiquidityProportionalRequest({
            pool: address(orchestratorV3.balancerPool()),
            maxAmountsIn: maxAmountsIn,
            sender: address(orchestratorV3),
            recipient: address(orchestratorV3),
            userData: ""
        });

        // Query the router
        uint256 bptAmountOut = orchestratorV3.balancerRouter().queryAddLiquidityProportional(req);
        console.log("Query result - expected LP tokens:", bptAmountOut);

        assertGt(bptAmountOut, 0, "Should get positive LP tokens from query!");
